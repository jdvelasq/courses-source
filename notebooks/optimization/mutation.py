        ...
        if "ES_sigma" in population[0].keys():
            ...
            if isinstance(population[0].ES_sigma, (int, float)):
                ...
                for individual in population:
                    ...
                    #
                    individual.ES_sigma = individual.ES_sigma * np.exp(tau * np.random.normal(size=n_dim))
                    individual.ES_sigma = np.where(individual.ES_sigma < self.sigma_max, individual.ES_sigma, self.sigma_max)
                    individual.ES_sigma = np.where(individual.ES_sigma > self.sigma_min, individual.ES_sigma, self.sigma_min)                    
                    #
